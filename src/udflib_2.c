...
double EXPORT _NotNull(double* a)
{
	return (*a);
}
...
/* ==================================================
 _CurDate  current date
 =================================================== */


char* EXPORT _CurDate()
{
	...
	if (!localtime_r(&timeVal, timeInfo)) //conversion failed
		return NULL;
#endif
//timeInfo points to a buffer with broken down time and date
	char* strDate = (char*)malloc(64);
	if (!strDate) return NULL;
	if (!strftime(strDate, 63, "%d.%m.%Y", timeInfo)) //string buffer too short, nothing written
		return NULL;
	return strDate;
}

// тази функция трябва да върне датата и времето в стринг в формат 04.10.2010 18:28:35

char* EXPORT _CurDateTime()
{
	struct tm* timeInfo = NULL;
	time_t timeVal = time(NULL);
#ifdef WIN32
...
#endif
//timeInfo points to a buffer with broken down time and date
	char* strDateTime = (char*)malloc(64);
	if (!strDateTime) return NULL;
	if (!strftime(strDateTime, 63, "%d.%m.%Y %H:%M:%S", timeInfo)) //string buffer too short, nothing written
		return NULL;
	return strDateTime;
}


// тази функция трябва да върне датата и времето в  формат double   40455.7765509606
double  EXPORT  _Now()
{
	return ((double)time(NULL));
}

// закръгля числото r  до places знака след запетаята математично закръгляне

double EXPORT  _Round(double* r, unsigned* places)
{
	double off=pow(10, *places);
	return ((int)round((*r) * off)) / off;
}
...
double EXPORT  _Discount(double* Value, double* Percent)
{
...
// дължината на стринга s

int EXPORT  _StrLen(char* s)
{
